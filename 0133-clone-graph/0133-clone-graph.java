...
// daily leetcode challenge -> 8th April
...
//         // new node with same value as given node
...
//             // check if node visited or not, if not visited then null
//             if(vis[i.val] == null){
//                 // if not visited then create a new node, and add it to the neighbour of prev node
//                 Node newNode = new Node(i.val);
//                 copy.neighbors.add(newNode);
//                 dfs(i, newNode, visited);
//             }else{
                
// //  if that node is already visited, retrieve that node from visited array and add it as the adjacent node of prev c
